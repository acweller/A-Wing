...
class MembroEsquadraoController {
   ...
   public async getOne(req: Request, res: Response): Promise<any>{
      const { id } = req.params;
      const membro_esquadrao = await pola.query('SELECT * FROM membro_esquadrao WHERE cod_esquadrao = ?', [id]);
   ...
   public async delete(req: Request , res: Response): Promise<any>{
      ...
      const membro_esquadrao = await pola.query('DELETE FROM membro_esquadrao WHERE cod_personagem =?',str[0]+'and cod_esquadrao = ?'+str[1]);
